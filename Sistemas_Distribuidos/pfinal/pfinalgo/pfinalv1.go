	...
	for ; full < Shelves; {
		...
		for i := 0; i < Shelves; {
			...
			if !reg[row] { //if that shelf has not been chosen yet
				...
				for j := 0; j < Positions; j++{ //Hasta que un regalo que aún esté disponible se elija 
					pos := (n + j) % Positions //Paso al siguiente cuando esté vacío
					//fmt.Println(n,j,Shelves,pos)
					if !(*caches)[tag][row][pos] { //Compruebo en mi caché si el regalo está disponible
						stash[row][pos] = false //Lo retiro del almacén
