	...
	for {
		...
		case msg := <-messages: // Receiving messages
			if strings.Index(msg, "!list") > -1 {
				...
				for cli, v := range clients { // Sending to myself
					if !compareIP(v, msg) {
						cli <- list
					}
				}
			} else if strings.Index(msg, "!private") > -1 {
				// !private-v-127.0.0.1:45078-127.0.0.1:45078
				// hacer comprobación de que todo va bien si no pone bien la cadena
				for cli, v := range clients {
					dest := strings.Split(msg, "-")[1] + "-" + strings.Split(msg, "-")[2]
					if (strings.Index(v, dest)) > -1 { //Esta en la lista, necesito añadir otra comprobación extra
						// Inicio chat privado
						fmt.Println(v)
						cli <- "Chat privado"
						privatechats[v] = dest
					} else {
						cli <- "Error.Cliente no existe\nFormato: !private-name-ip\nPuedes hacer !list para consular los clientes"
					}
				}
				fmt.Println(privatechats)
